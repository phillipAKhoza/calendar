...
function CalendarPicker(element, options) {
  ...
  this.attandance = document.createElement("p");
  ...
  this.currentDate.id = "date";
  this.attandance.id = "attandance";
...
CalendarPicker.prototype._getDaysInMonth = function (month, year) {
  ...
  days.forEach(function populateWeek(item, index, arr) {
    var dateDetails = arr[index].toString().split(" ");

    this.dayAsText = dateDetails[0];
    this.monthAsText = dateDetails[1];
    this.dateAsText = dateDetails[2];
    this.yearAsText = dateDetails[3];
    week.push(this.dateAsText);
    console.log(this.dateAsText);
  }); // day in days
...
/**
  Sets the current date as readable text in their own variables
 */
...
/**
 Inserts the calendar into the wrapper and adds eventListeners for the calender-grid.
 */
...
CalendarPicker.prototype._insertHeaderIntoCalendarWrapper = function () {
  ...
  this.calendarHeader.appendChild(this.currentDate);

  this.calendarWrapper.appendChild(this.calendarHeader);
};

/**
Inserts the calendar-grid header with all the weekdays.
 */
...
/**
Adds the "Previous" and "Next" arrows on the side-navigation.
...
/**
Adds all the days for current month into the calendar-grid.
...
CalendarPicker.prototype._insertDaysIntoGrid = function () {
  ...
  arrayOfDays.forEach((date) => {
    ...
    if (dateIsBetweenAllowedRange) {
      ...
      dateElement.style.background = "#E34234";
    } else {
...
/**
 Updates the core-values for the calendar based on the new month and year
...
/**
  A "listener" that lets the user do something everytime the value changes.
 */
